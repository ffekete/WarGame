...
public class BattleMapGrassSpreadDecorator implements Decorator {
    ...
    public BattleMapGrassSpreadDecorator(AssetManager assetManager, StageElementsStorage stageElementsStorage) {
        this.assetManager = assetManager;
        treeVariations = ImmutableList.<Texture>builder()
                .add(assetManager.get("GrassDecoration.png", Texture.class))
                .add(assetManager.get("GrassDecoration02.png", Texture.class))
                .add(assetManager.get("GrassDecoration03.png", Texture.class))
                .build();
        ...
        for (int i = 0; i < newMap.length; i++) {
            for (int j = 0; j < newMap[0].length; j++) {
                if (newMap[i][j] == 1) {
                    ...
                    AnimatedImage grass = new AnimatedImage(new TextureRegion(treeVariations.get(rnd)), 0.5f);
                    grass.setPosition(i, j);
                    grass.setSize(1, 1);
                    stageElementsStorage.groundLevel.addActor(grass);
