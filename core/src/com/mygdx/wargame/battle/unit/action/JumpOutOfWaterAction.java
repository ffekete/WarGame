...
public class JumpOutOfWaterAction extends MoveToAction {
    ...
    public JumpOutOfWaterAction(float sx, float sy, float ex, float ey, AbstractMech mech, AssetManager assetManager, StageElementsStorage stageElementsStorage) {
        this.sx = sx;
        this.sy = sy;
        this.ex = ex;
        this.ey = ey;
    ...
    protected void update(float percent) {
        if(percent < 0.999) {
            if(firstRun) {
                mech.addAction(new AddActorAction(stageElementsStorage.mechLevel, new Splash(assetManager, mech.getX(), mech.getY() + 0.01f)));
                firstRun = false;
            }
