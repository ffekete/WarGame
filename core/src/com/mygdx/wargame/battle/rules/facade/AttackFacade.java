...
//        }
    ...
    public void checkHeatDamage(com.mygdx.wargame.common.component.weapon.Weapon weapon) {
        if (GameState.selectedMech.getHeatLevel() > 100) {
            if (new Random().nextInt(100) - (GameState.selectedPilot.hasPerk(Perks.Hazardous) ? 10 : 0) >= 80) {
                System.out.println("Heat");
                heatDamageCalculator.calculate(GameState.selectedMech, weapon, messageQue);
            }
        }
    }
    ...
    public WeaponStabilityDecreaseCalculator getWeaponStabilityDecreaseCalculator() {
        return weaponStabilityDecreaseCalculator;
    }

    public EvasionCalculator getEvasionCalculator() {
