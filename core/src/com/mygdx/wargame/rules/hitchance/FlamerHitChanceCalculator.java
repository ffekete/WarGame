...
public class FlamerHitChanceCalculator implements HitChanceCalculator {

    @Override
    public int calculate(Pilot pilot, Mech mech, Mech target, Weapon weapon) {

        int baseHitChance = pilot.getSkills().get(Skill.Flamer) * 5;
        ...
        Optional<Integer> targetingModuleModifiers = mech.getAllComponents()
                ...
                .map(c -> c.getAdditionalAccuracy(WeaponType.Flamer))
                .reduce(Integer::sum);
        ...
        if (pilot.hasPerk(Perks.FlamerExpert)) {
            baseHitChance += 5;
        }
